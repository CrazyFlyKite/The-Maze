...
class Game:
	...
	def setup_canvas(self, grid: Grid) -> None:
		...
		for y, row in enumerate(grid):
			for x, cell in enumerate(row):
				match cell:
					...
					case CellType.WALL:
						wall_id: int = self.canvas.create_rectangle(
							x * TILE, y * TILE,
					...
					case CellType.COIN:
						coin_id: int = self.canvas.create_rectangle(
							x * TILE + 3, y * TILE + 3,
					...
					case CellType.DOOR:
						door_id: int = self.canvas.create_rectangle(
							x * TILE + 1, y * TILE + 1,
					...
					case CellType.EXIT:
						exit_id: int = self.canvas.create_rectangle(
							x * TILE + 1, y * TILE + 1,
							x * TILE + TILE - 1, y * TILE + TILE - 1,
